const addScoreToLS = (cellID: keyof ScoreSheet, sum: number) => {
...
const checkIncomingNumbers = (cell: HTMLTableCellElement) => {
    ...
    switch (idrecieved) {
        ...
        case `${activePlayerLS}Two Pair`:
            if (!(currentScore.length === 4)) { return console.log('It says pairs mate, sorry.') }
            console.log(diceSorted)
            if (!(diceSorted[0] == diceSorted[1] && diceSorted[2] == diceSorted[3])) { return console.log('It says pairs mate, sorry.') }
        ...
        case `${activePlayerLS}Full House`:
            if (!((diceSorted[0] == diceSorted[1] && diceSorted[1] == diceSorted[2] && diceSorted[3] == diceSorted[4])) || ((diceSorted[0] == diceSorted[1] && diceSorted[2] == diceSorted[3] && diceSorted[3] == diceSorted[4]))) { return console.log('sorry mate, thas not a full house')}
            addScoreToLS('Full House', sum)
            saveScore(cell, sum)

        break;
        case `${activePlayerLS}Small Straight`:
            if (!(currentScore.includes(1) && currentScore.includes(2) && currentScore.includes(3) && currentScore.includes(4)) || (currentScore.includes(1) && currentScore.includes(2) && currentScore.includes(3) && currentScore.includes(4) && currentScore.includes(5))) { return console.log('This is not a small straight')}
            sum = 15;
        ...
        case `${activePlayerLS}Large Straight`:
            if (!(currentScore.includes(2) && currentScore.includes(3) && currentScore.includes(4) && currentScore.includes(5)) || (currentScore.includes(2) && currentScore.includes(3) && currentScore.includes(4) && currentScore.includes(5) && currentScore.includes(6))) { return console.log('This is not a large straight') }
            sum = 20;
            addScoreToLS('Large Straight', sum)
            saveScore(cell, sum)
        break;
        case `${activePlayerLS}Yahtzee`:
            if (!((movesMade.length === 1) && (currentScore.length === 5 || currentScore.length === 6) && (currentScore.every(die => (die === 1) || (die === 2) || (die === 3) || (die === 4) || (die === 5) || (die === 6))))) { return console.log('this is not a yathzee') }
            sum = 100;
