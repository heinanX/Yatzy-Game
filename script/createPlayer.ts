...
const createPlayers = () => {
    ...
    playerCell.setAttribute('class', `highlight${player.name}`)
    ...
    playersFromLS.forEach(element => {
        ...
        scoreData.setAttribute('id', `${element.name}${category.name}`)
    ...
    const total = document.querySelector('.Total') as HTMLTableCellElement
